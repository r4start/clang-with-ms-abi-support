...
void Parser::TryAnnotateIdentifier() {
  assert(getLangOpts().CPlusPlus && getLangOpts().MicrosoftExt && 
         "kw___identifier can be used only when MicrosoftExt and C++ enabled");
  assert(Tok.is(tok::kw___identifier) && "Not '__identifier'!");
  
  SourceLocation StartLoc = ConsumeToken(); // consume __identifier
  BalancedDelimiterTracker T(*this, tok::l_paren);

  // __identifier expressions are always parenthesized.
  if (T.expectAndConsume(diag::err_expected_lparen_after, "__identifier"))
    return;

  // TODO: make __identifier(<not C++ keyword>) work only with 
  // pragma warning(disable: 4483)
  IdentifierInfo* Ident;
  // for __identifier("int") usage
  if (Tok.is(tok::string_literal)) {
    ExprResult IdentifierExpr(ParseStringLiteralExpression());
    if (IdentifierExpr.isInvalid())
        return;
    T.consumeClose();
    StringLiteral *Identifier = IdentifierExpr.takeAs<StringLiteral>();
    StringRef Name = Identifier->getString();
    Ident = PP.getIdentifierInfo(Name, true);
  }
  // for __identifier(int) usage
    ...
    Ident = PP.getIdentifierInfo(Name, true);
      ...
      const char* Id = getKeywordIdentifier(Tok.getKind());
      if (!Id) {
  ...
  Tok.setKind(tok::annot___identifier);
  ...
  // Set kind back to plain tok::identifier to avoid hundreds code modifications
